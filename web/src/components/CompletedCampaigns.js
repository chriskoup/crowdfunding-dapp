...
const CompletedCampaigns = () => {
  ...
  useEffect(() => {
    const init = async () => {
      if (window.ethereum) {
        ...
        const ids = await contractInstance.methods.getCompletedCampaigns().call();
  ...
  return (
    <div className="mt-6">
      <h2 className="text-xl font-bold mb-2">Ολοκληρωμένες Καμπάνιες</h2>
      {campaigns.length === 0 ? (
        <p>Καμία ολοκληρωμένη καμπάνια.</p>
      ) : (
        campaigns.map((c) => (
          <div key={c.id} className="border p-4 rounded mb-4 bg-gray-100">
            ...
            <p><strong>Συγκεντρωμένες Μετοχές:</strong> {c.pledgesCount} / {c.pledgesNeeded}</p>
          </div>
        ))
      )}
    </div>
